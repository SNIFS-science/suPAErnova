...
def train_step(
    ...
    if model.params["train_noise"]:
        ...
        noise_vary = model.params["noise_scale"] * np.abs(
            np.random.normal(0.0, train_data["sigma"]).astype(np.float32),
        )
    else:
    ...
    if model.params["vary_mask"]:
        ...
        for i in range(mask_vary.shape[0]):
            ...
            mask_vary[i, :nmaski] = 0.0
            mask_vary[i : i + 1, :nii] = mask_vary[
                i : i + 1,
                np.random.rand(nii).argsort(),
            ]
            # np.take(mask_vary[i:i+1,:nii], np.random.rand(nii).argsort(), axis=1, out=mask_vary[i:i+1,:nii])

    mask_vary[~train_data["mask_sn"]] = 0.0
    mask_vary[~train_data["mask_spectra"][..., 0]] = 0.0
    ...
    if model.params["train_time_uncertainty"]:
        ...
        dtime = np.random.normal(0, train_data["dphase"] / 50)[:, None, None].astype(
            np.float32,
        )

    if epoch == 0:
        print(
            "Number of training SN to use for training: ",
            np.sum(train_data["mask_sn"]),
        )
    ...
    training_loss_terms = [
        training_loss_terms[ii] / nbatches for ii in range(len(training_loss_terms_b))
    ]
...
def test_step(model, data):
    ...
    mask_vary[~data["mask_sn"]] = 0.0
    mask_vary[~data["mask_spectra"][..., 0]] = 0.0
...
def calculate_mean_parameters_batches(model, data, nbatches):
    ...
    for batch in range(nbatches):
        ...
        dm = mask_vary[inds_batch, 0, 0] == 1.0
...
def train_model(train_data, val_data, test_data, model):
    ...
    val_loss_hist = np.zeros((
        model.params["epochs"] // model.params["val_every"],
        ncolumn_loss,
    ))
    test_loss_hist = np.zeros((
        model.params["epochs"] // model.params["val_every"],
        ncolumn_loss,
    ))
    ...
    for epoch in range(model.params["epochs"]):
        ...
        if epoch % model.params["val_every"] == 0:
            ...
            if model.params["scheduler"].upper() == "EXPONENTIAL":
                print(
                    "Learning rate is currently: {:.6f}".format(
            ...
            if model.params["optimizer"].upper() == "ADAMW":
                print(
                    f"Weight decay is currently: {optimizer.weight_decay:.6f}",
                )

            previous_val_decrease = (
                val_iteration - val_iteration_best
            )  # number of validation iterations since last loss decrease
...
def save_model(model, params, data, nbatches, is_best=False) -> None:
    if params["savemodel"]:
        fname = "AE_kfold{:d}_{:02d}Dlatent_layers{:s}_{:s}".format(
            params["kfold"],
            params["latent_dim"],
        ...
        if model.params["physical_latent"]:
            ...
            model_save = autoencoder.AutoEncoder(
                params,
                training=False,
