...
if __name__ == "__main__":
    ...
    data_out["times"] = (
        np.zeros((n_sn, n_timestep, 1), dtype=np.float32) - 100
    )  # set missing data to -100
    ...
    for i, idi in enumerate(np.unique(data["ID"])):
        ...
        for ispec in range(n_speci):
            ...
            if keep_min != -1.0:
                ...
                data_out["mask"][i, ispec, ind_keep_min:ind_keep_max] = (
                    True  # mark as valid spectrum
                )
    ...
    if verbose:
        print("number of spectra each: ", n_spec_each)

    from astropy.cosmology import WMAP7 as cosmo

    data_out["luminosity_distance"] = cosmo.luminosity_distance(
        data_out["redshift"],
    ).value
