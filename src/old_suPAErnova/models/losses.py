...
def compute_loss_ae(model, x, cond, sigma, mask):
    ...
    noise_floor = 0.0  # Generally added when data loaded, so set to 0
    ...
    if model.params["loss_fn"].upper() == "RMSE":
        loss = tf.reduce_mean(
            tf.reduce_sum((x - x_pred) ** 2 * mask, axis=(-2, -1))
    ...
    if model.params["loss_fn"].upper() == "NGLL":
        loss = tf.reduce_mean(
            tf.reduce_sum(
                (
                    tf.math.log(sigma**2 * mask + noise_floor) / 2
                    + (x - x_pred) ** 2 / (2 * sigma**2 + noise_floor)
                )
                * mask,
                axis=(-2, -1),
            ),
    ...
    if model.params["loss_fn"].upper() == "MAGNITUDE":
        cond = x_pred >= 0.0
        mag_loss = (
            tf.math.abs(tf.math.log((x_pred + 1e-9) / (x * mask + 1e-9))) * mask
        )  # 1./sigma * mask
        mag_loss_amp = (
        ...
        loss_amp = tf.reduce_mean(
            tf.abs(
                tf.reduce_sum(tf.where(cond, mag_loss_amp, nan_error), axis=(-2, -1)),
            ),
        )

        loss += loss_amp

    loss_recon = loss * 1.0  # reconstruction loss
    ...
    if model.params["iloss_covariance"]:
        ...
        std_z = tf.sqrt(
            tf.reduce_sum(z_cov**2, axis=0) / num_kept,
...
def get_compute_MAP():
    ...
    def compute_apply_gradients_MAP(model, x, trainable_params, optimizer):
        ...
        optimizer.apply_gradients(zip(gradients, trainable_params, strict=False))
